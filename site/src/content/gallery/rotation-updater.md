...
known_gaps:
  - symptom: "Manim の callback updater `(mobj, dt)` は直接実行しないが、`rotate_about_origin(dt)` の累積結果を `rotateUpdater` で `rotation` アニメーションへ展開している。"
    layer: dsl
    impact: low
    workaround: "`Line(ORIGIN, LEFT)` を Manim frame scale の67.5px線分・既定stroke幅へ展開し、`line_moving.set_color(YELLOW)` は Manim `YELLOW` 定数に合わせる。`rotateUpdater rate=1 duration=2s` と `rate=-1` で、Manim の rad/s `dt` 累積を約 114.592 度の線形回転と逆回転に変換する。"
