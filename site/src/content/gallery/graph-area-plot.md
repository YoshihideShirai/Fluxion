...
known_gaps:
  - symptom: "Manim の `Axes.get_riemann_rectangles` と `Axes.get_area(... bounded_graph=...)` は、`dataRiemannRects` と `dataArea` が axes データ座標から静的 geometry を生成している。"
    layer: dsl
    impact: low
    workaround: "`axes`、`plot`、`dataLine`、`dataRiemannRects`、`dataArea` で公式式から各 geometry を生成する。"
...
math x_label "x" at 447,225 size=28 fill="#FFFFFF"
math y_label "y" at -409,-253 size=28 fill="#FFFFFF"
