...
known_gaps:
  - symptom: "ZoomedScene の副カメラは runtime の `cameraView` と source frame の再描画で表現しているため、Manim の Cairo camera object と完全に同一の内部表現ではない。"
    layer: runtime
    impact: low
    workaround: "`cameraView zoom_capture sourceFrame=frame` が元シーンを毎フレーム切り出して zoom display 内へ描画し、frame の移動・非等方 scale・display の pop-out/scale/reverse pop-out に追従する。"
    closure_condition: "Manim の camera object API 全体を runtime に公開した場合に内部表現の差分も解消できる。"
