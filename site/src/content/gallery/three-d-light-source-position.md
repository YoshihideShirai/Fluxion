...
known_gaps:
  - symptom: "3D renderer と実際の light_source shading は未実装だが、公式の ThreeDAxes と球面 Surface checkerboard は `threeDAxes` / `sphereSurface` helper から投影済み face 群へ展開している。"
    layer: runtime
    impact: medium
    workaround: "`ThreeDAxes()` を `threeDAxes` の斜投影 line/tick/tip 群で描き、`Surface(..., v_range=[0, TAU], u_range=[-PI/2, PI/2], checkerboard_colors=[RED_D, RED_E], resolution=(15,32))` を `sphereSurface` の shaded checkerboard path 群に展開する。公式 sphere 半径 1.5 を Manim 16:9 frame scale の 67.5px/unit で 101.25px とし、`light_source.move_to(3*IN)` に合わせて上中心寄りのハイライト、sheen、反対側の terminator shadow を重ねる。"
...
threeDAxes axes at 0,28 xRange=-2.555,4,1 yRange=-3.1,4,1 zRange=-3.675,4,1 xBasis=-56.75,25.5 yBasis=87.75,13.25 zBasis=0,-55.25 stroke="#FFFFFF" strokeWidth=4 tickSize=10 tickStrokeWidth=2 includeTicks=true includeTips=true

sphereSurface sphere_mesh radius=104 resolution=15,32 fillA="#E65A4C" fillB="#CF5044" light=0,-0.35,1
