...
known_gaps:
  - symptom: "Brace shape and labels are generated from the primitive, but SVG mobject point sampling / browser text metrics may still differ slightly from Manim."
    layer: renderer
    impact: low
    workaround: "Manim の Brace source と同じ path template / `default_min_width=0.90552` / `linear_section_length` モデルを使った塗りつぶし brace と、`get_text` / `get_tex` 相当の primitive label を tip 近傍へ配置する。公式 `self.add(line, dot, dot2, b1, b2, b1text, b2text)` と同じく白い dot を orange line の上に描く。"
