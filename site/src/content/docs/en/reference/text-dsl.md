...
- `threeDAxes` only: `xRange=<min,max,step>`, `yRange`, `zRange`, `xLength`, `yLength`, `zLength`, `xBasis=<x,y>`, `yBasis=<x,y>`, `zBasis=<x,y>`, `includeTicks`, `includeTips`; emits projected line/tick/tip groups for Manim `ThreeDAxes()` default ranges and axis lengths (`xLength=10.5`, `yLength=10.5`, `zLength=6.5`). When `phi` / `theta` / `gamma` are provided, each axis endpoint and tick is placed through the Manim `ThreeDCamera` perspective projection.
...
- `tracedPath` only: either `x`, `y`, `from`, `to`, `samples`, `close=true|false`, `sampling=fixed|frame`, `sampleStep`, `smoothing=linear|smooth` for a generated parametric `path` plus `bindPath` updater, or `target=<node-id>`, `start=<time>`, `samples`, `sampling=fixed|frame` to rebuild the target node's center history from the timeline at seek time, similar to Manim `TracedPath(mobject.get_center)`. For expression traces, `sampling=frame` with `sampleStep` grows the point count over the animated tracker range. For target traces, `sampling=frame` uses the document fps up to `samples`, so partial traces grow with elapsed frames.
...
- `Create(id, lagRatio=1)`: creates the node and emits `effect=create`. Drawable leaves animate `geometry.drawProgress` using Manim-like submobject lag timing.
- `Write(id)`: creates writable leaves with `geometry.writeProgress=0`, emits `effect=write`, and reveals each leaf with Manim-like `lag_ratio=min(4 / n, 0.2)` timing and the default `linear` rate when no easing is specified.
