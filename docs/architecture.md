...
Mermaid や PlantUML のように、ブラウザ上で短いテキストを編集し、その場で `.vanim.json` に変換して再生できる入力方式も提供する。
...
構文エラーは `.vanim.json` の生成前に検出し、行番号・列番号・期待される token を返す。生成後は既存の JSON schema で検証する。
